...
void StereoSLAMNode::initialize_subscribers() 
{
    // Image subscribers
    left_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(shared_ptr<rclcpp::Node>(this), "camera/left/image_rect");
    right_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(shared_ptr<rclcpp::Node>(this), "camera/right/image_rect");

    // Sync left and right images
    syncApproximate = std::make_shared<message_filters::Synchronizer<approximate_sync_policy> >(approximate_sync_policy(10), *left_sub, *right_sub);
    syncApproximate->registerCallback(&StereoSLAMNode::grab_data, this);
}
...
void StereoSLAMNode::grab_data(const ImageMsg::SharedPtr msg_Left, const ImageMsg::SharedPtr msg_Right)
{
    try
    {
        cv_ptrLeft_ = cv_bridge::toCvShare(msg_Left);
        cv_ptrRight_ = cv_bridge::toCvShare(msg_Right);

        current_pose_ = SLAM_->TrackStereo(cv_ptrLeft_->image, cv_ptrRight_->image, Utility::StampToSec(msg_Left->header.stamp));
        
        Sophus::SE3f Twc = SLAM_->GetCamTwc();
        if(Twc.translation().array().isNaN()[0] || Twc.rotationMatrix().array().isNaN()(0,0)) // do not publish if the pose is Nan
            return;
        ...
        publish_tracking_img(SLAM_->GetCurrentFrame(), msg_Left->header.stamp);
...
void StereoSLAMNode::publish_camera_pose(const Sophus::SE3f& Tcw_SE3f, const rclcpp::Time& msg_time)
{
...
void StereoSLAMNode::publish_tracked_points(const std::vector<ORB_SLAM3::MapPoint*>& tracked_points, const rclcpp::Time& msg_time)
{
    sensor_msgs::msg::PointCloud2 cloud = mappoint_to_pointcloud(tracked_points, msg_time);
    tracked_mappoints_pub_->publish(cloud);
}

void StereoSLAMNode::publish_all_points(const std::vector<ORB_SLAM3::MapPoint*>& all_points, const rclcpp::Time& msg_time)
{
    sensor_msgs::msg::PointCloud2 cloud = mappoint_to_pointcloud(all_points, msg_time);
    all_mappoints_pub_->publish(cloud);
}

void StereoSLAMNode::publish_tracking_img(cv::Mat image, const rclcpp::Time& msg_time)
{
...
void StereoSLAMNode::publish_kf_markers(const std::vector<Sophus::SE3f> vKFposes, const rclcpp::Time& msg_time)
{
...
void StereoSLAMNode::publish_odometry(const Sophus::SE3f& Tcw_SE3f, const rclcpp::Time& msg_time)
{
...
sensor_msgs::msg::PointCloud2 StereoSLAMNode::mappoint_to_pointcloud(std::vector<ORB_SLAM3::MapPoint*> map_points, const rclcpp::Time& msg_time)
{
...
void StereoSLAMNode::setup_services()
{
    save_map_service_ = this->create_service<std_srvs::srv::Empty>(
        "save_map", std::bind(&StereoSLAMNode::save_map_srv, this, std::placeholders::_1, std::placeholders::_2));

    save_trajectory_service_ = this->create_service<std_srvs::srv::Empty>(
        "save_traj", std::bind(&StereoSLAMNode::save_trajectory_srv, this, std::placeholders::_1, std::placeholders::_2));
}

bool StereoSLAMNode::save_map_srv(const std::shared_ptr<std_srvs::srv::Empty::Request> request,
...
bool StereoSLAMNode::save_trajectory_srv(const std::shared_ptr<std_srvs::srv::Empty::Request> request,
                                       std::shared_ptr<std_srvs::srv::Empty::Response> response)
{
